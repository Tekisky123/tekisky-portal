...
function ViewStudent() {
    ...
    useEffect(() => {
        axios
            .get(`https://tekisky-portal-e544.onrender.com/student/getstudent`)
            .then((response) => {
    ...
    const deleteStudent = (e, id) => {
        e.preventDefault()
        axios.get(`https://tekisky-portal-e544.onrender.com/student/${id}`)
            .then((response) => {
    ...
    const details = (id) => {
        navigate(`/getOneStudent/${id}`)
    }

    // const filter = (event) => {
    //     setRecords(data.filter(student => student?.personalDetails?.name.toLowerCase().includes(event.target.value)))
    //     // setRecords(data.filter(student => student?.courseDetails?.courseName.toLowerCase().includes(event.target.value)))
    // }

    const filter = (event) => {
        const searchText = event.target.value.toLowerCase();

        setRecords(data.filter(student =>
            student?.personalDetails?.name.toLowerCase().includes(searchText) ||
            student?.courseDetails?.courseName.toLowerCase().includes(searchText)
        ));
    }

    return (
        <div className={style.mainDivStd}>
            <div className={style.searchMainDiv}>
                <div className={style.searchDiv}>
                    <img src="\public\search.png" alt="" className={style.searchImg} />
                    <input type="text" onChange={filter} className={style.searchInput} placeholder="Search Student Here" />
                </div>
            </div>
            <br />
            {records.map((student, index) => (
                <div className={style.cartDiv} key={index}>
                    <div className={style.cartDivTable}>
